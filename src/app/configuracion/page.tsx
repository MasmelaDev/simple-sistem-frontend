...
const Configuracion = () => {
    ...
    const [domiciliary, setDomiciliary] = useState<string>('')
    const [isPending, startTransition] = useTransition()

    const neighborhoodSubmit = () => {
        startTransition(async () => {
            await createNeighborhood(neighborhood)
            setNeighborhood('')
        })
    }

    const domiciliarySubmit = async () => {
        startTransition(async () => {
            await createDomiciliary(domiciliary)
            setDomiciliary('')
        })
    }
    return (
        <main className="pt-16 flex flex-col items-center justify-center min-h-screen ">
            <aside className="flex flex-col xs:flex-row gap-5 p-3 absolute top-16 w-full justify-center">
                <div className="sm:w-64">
                    ...
                    <button
                        onClick={neighborhoodSubmit}
                        className="bg-green-400 text-white mt-2 py-1 px-2 rounded-md font-medium"
                    >
                        {isPending ? (
                            <div role="status">
                                <svg
